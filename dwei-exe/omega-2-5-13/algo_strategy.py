...
2. 3-Phase Attack System (ALWAYS rebuilds defenses after each attack):
   - Phase 1: REMOVE turrets at blocking_turret_position2 (MP >= escalating threshold)
   - Phase 2: Deploy scouts with MODIFIED ESCALATING waves (Wave 1: 5→7→stays at 7, MP threshold +2 per cycle)
   - Phase 3: ALWAYS rebuild all blocking_turret_position2 turrets and reset all flags (next turn)
3. MODIFIED ESCALATING ATTACK WAVES: 
...
4. Hard reset to normal defensive state before each new attack cycle
"""
...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.primary_turrets = [[1,12],[1,13],[2,12],[2,13],[3,12],[3,13],[4,12],[4,13],[5,13],[6,13],[7,13],[8,13],[9,13],[10,13],[11,13],[12,13],[13,13],[14,13],[15,13],[16,13],[17,13],[18,13],[19,13],[20,13],[21,13],[22,13],[23,13],[24,12],[24,13],[25,12],[25,13],[26,12],[26,13]]
        ...
        gamelib.debug_write('MODIFIED ESCALATING ATTACKS INITIALIZED: Wave 1 goes 5→7→stays at 7, MP threshold increases +2 per cycle')
    ...
    def on_turn(self, turn_state):
        ...
        gamelib.debug_write('Turn {} - Dynamic Escalating Defense Strategy (Attack threshold: MP>={}, Direction: {}, Wave 1: {} scouts)'.format(
    ...
    def execute_defensive_strategy(self, game_state):
        ...
        if not self.ready_for_scout_rush and not self.turret_removed_for_attack and not self.attack_path_cleared:
            ...
            gamelib.debug_write('🎯 DYNAMIC ANALYSIS: Updated attack direction to {} (not in active attack cycle)'.format(self.current_attack_direction.upper()))
        else:
            phase_status = []
            ...
            gamelib.debug_write('🎯 DYNAMIC ANALYSIS: Skipped (Active: {}) - maintaining {} direction, blocking protection ACTIVE'.format(
    ...
    def analyze_enemy_and_set_attack_direction(self, game_state):
        """
        DYNAMIC ATTACK STRATEGY: Analyze enemy defenses and set attack direction/positions
        """
        try:
            # Analyze middle attack viability
            middle_viable = self.should_attack_middle(game_state)
            
            if middle_viable:
                self.set_center_attack_configuration()
                gamelib.debug_write('🎯 DYNAMIC ATTACK: CENTER attack selected (weak middle defense)')
            else:
                ...
                if attack_left:
                    ...
                    gamelib.debug_write('🎯 DYNAMIC ATTACK: LEFT attack selected (Left: {}T/{}U vs Right: {}T/{}U)'.format(
                        left_side_analysis['total_turrets'], left_side_analysis['upgraded_turrets'],
                        right_side_analysis['total_turrets'], right_side_analysis['upgraded_turrets']))
                else:
                    self.set_right_attack_configuration()
                    gamelib.debug_write('🎯 DYNAMIC ATTACK: RIGHT attack selected (Left: {}T/{}U vs Right: {}T/{}U)'.format(
                        left_side_analysis['total_turrets'], left_side_analysis['upgraded_turrets'],
                        right_side_analysis['total_turrets'], right_side_analysis['upgraded_turrets']))
        except Exception as e:
            # Fallback to left attack if analysis fails
            gamelib.debug_write('ERROR in enemy analysis, defaulting to LEFT attack: {}'.format(str(e)))
            self.set_left_attack_configuration()
    def should_attack_middle(self, game_state):
        """
        Check if we should attack down the middle by analyzing columns 7-19
        Returns True if more than 8 columns have only 1 turret or less
        """
        ...
        gamelib.debug_write('MIDDLE ANALYSIS: {}/13 columns weak (need >8 for middle attack)'.format(weak_columns))
        return weak_columns > 8  # Attack middle if more than 8 weak columns
    ...
    def execute_attack_strategy(self, game_state):
        ...
        if mp >= self.min_attack_mp and not self.ready_for_scout_rush and not self.turret_removed_for_attack:
            ...
            gamelib.debug_write('🚀 DYNAMIC ESCALATING SCOUT RUSH ACTIVATED! 🚀')
            ...
            gamelib.debug_write('DYNAMIC STRATEGY: Analyzes enemy defenses each turn → CENTER/LEFT/RIGHT attack + Modified escalation!')
