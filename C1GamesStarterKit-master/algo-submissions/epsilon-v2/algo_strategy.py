...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.primary_turrets = [
            ...
            [11,7], [12,7], [15,7], [16,7], [19,7], [20,7], [21,7],
        ...
        self.secondary_turrets = [[10,7], [13,7], [14,7], [17,7]]
        
        # Support positions (build after all turrets complete)
        self.support_positions = [
            [6,12], [7,11], [8,10], [7,12], [8,11], [8,12], [9,11], [9,10]
        ]
    ...
    def execute_defensive_strategy(self, game_state):
        ...
        """
        # Priority 1: Ensure all primary turrets are built/replaced
        self.build_primary_turrets(game_state)
        
        # Priority 2: Build secondary turrets if primary is complete
        if self.primary_turrets_complete(game_state):
            self.build_secondary_turrets(game_state)
        
        # Priority 3: Build support structures if all turrets complete
    ...
    def build_support_structures(self, game_state):
        ...
        # Upgrade supports if we have extra SP
        sp = game_state.get_resource(SP)
        if sp >= 10:
            for location in self.support_positions[:4]:  # Upgrade first 4
                if game_state.contains_stationary_unit(location):
                    game_state.attempt_upgrade(location)
                    gamelib.debug_write('Upgraded support at {}'.format(location))
    ...
    def deploy_early_interceptors(self, game_state):
        """
        Deploy interceptors at specified positions during early game - every 2 turns
        """
        mp = game_state.get_resource(MP)
        turn = game_state.turn_number
        
        # Only deploy every 2 turns (turn 0, 2, 4, 6, etc.)
        if turn % 2 == 0 and mp >= 3:
            for position in self.interceptor_positions:
                if game_state.can_spawn(INTERCEPTOR, position):
                    ...
                    gamelib.debug_write('Deployed interceptor at {} (Turn {})'.format(position, turn))
                    break  # Deploy one at a time

    def execute_scout_rush(self, game_state):
        """
        Execute the scout rush: remove blocking turret and mass deploy scouts
        """
        mp = game_state.get_resource(MP)
        
        # Remove blocking turret if not already done
        if not self.turret_removed_for_attack:
            if game_state.contains_stationary_unit(self.blocking_turret_position):
                ...
                gamelib.debug_write('Removed blocking turret at {} for attack path'.format(self.blocking_turret_position))
        
        # Deploy all available MP as scouts
        if mp >= 8:  # Minimum viable rush
            scouts_to_deploy = min(mp, 30)  # Deploy up to 30 scouts
            
            if game_state.can_spawn(SCOUT, self.scout_attack_position, scouts_to_deploy):
                game_state.attempt_spawn(SCOUT, self.scout_attack_position, scouts_to_deploy)
                gamelib.debug_write('SCOUT RUSH: Deployed {} scouts at {}!'.format(scouts_to_deploy, self.scout_attack_position))
            else:
                # Deploy as many as possible
                actual_deployed = game_state.attempt_spawn(SCOUT, self.scout_attack_position, 1000)
                gamelib.debug_write('SCOUT RUSH: Deployed {} scouts at {}!'.format(actual_deployed, self.scout_attack_position))
